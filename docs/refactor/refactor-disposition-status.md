...
| T-35 LEGACY_INDIA reduction → 49 (acceptance met) | Phase 9-bis-2 final-cleanup | ~85 files relocated to `market_regions/india/legacy_v1/` via sys.modules-aliasing shims (sandbox engine, options/analytics/flow/historify/sandbox/analyzer/telegram/chart services, India-only blueprints, India-only DB modules, India-only utils). 36 region-neutral modules promoted to PROMOTED_CORE (utilities, generic services, region-neutral DB models). Final bucket counts: LEGACY_INDIA 49, PROMOTED_CORE 121, REGION_PLUGIN 168, COMPATIBILITY_SHIM 86. |
| Provider initial-funds discrepancy reconciliation | Phase 9-bis-2 final-cleanup | Confirmed as intentional discrepancy by design and documented in the provider source. |
| T-17 / T-18 frontend per-component migration closure | Phase 4-bis-2 (relocation closure) | Flow constants + 19 flow nodes + strategyMath.ts + strategyTemplates.ts relocated to `frontend/src/india_legacy/`. Acknowledged as INTENTIONALLY-INDIA-ONLY (encode India options grammar / NSE operations) — the relocation IS the answer. |
| T-24 full — physical doc relocation under `docs/api/v1/<section>/` | Phase 8-bis-2-2 | All 9 section dirs (`account-services/`, `analyzer-services/`, `market-calendar/`, `market-data/`, `options-services/`, `order-information/`, `order-management/`, `symbol-services/`, `websocket-streaming/`) + `rate-limiting.md` + `v2_errors.md` relocated to `docs/api/v1/` (and `v2/v2_errors.md`). Per-file redirect stubs at every old path preserve external integration deep links. `docs/api/README.md` slimmed to multi-lane top nav; `docs/api/v1/README.md` lifted from the old comprehensive top-level README. |
| T-23 full — physical relocation of v1 lane internals | Phase 9-bis-physical | Four-group migration per the playbook: Group A (3 schemas), Group B (`domain/translators.py` + `utils/constants.py`), Group C (40 endpoint route modules), Group D (`_v1_lane_guard.py`). All 44 files moved into `market_regions/india/legacy_v1/`. Re-export shims (Option B) at every original path preserve every existing import. `restx_api/__init__.py` (the v1 lane registry + before/after request hooks + sunset machinery) stays at the canonical location and rewires its namespace imports to the new path. `tests/contracts/test_v4_closing_invariants.py` + `test_v5_india_v2_default_on.py` + `test_v5_observability_labels_complete.py` updated to point at the new `_v1_lane_guard.py` location. Bucket counts: LEGACY_INDIA 211→167; REGION_PLUGIN 31→78; COMPATIBILITY_SHIM 7→10. Achieves the restx_api/* + domain/translators.py + utils/constants.py portion of the prompt's T-35 acceptance. |
...
| ~~`IndiaSandboxProvider._INITIAL_FUNDS` (₹10L) vs `sandbox/fund_manager.py` (₹1Cr) reconciliation~~ | ~~v9-bis-2 Phase 2-bis-2~~ | **CLOSED 2026-05-02 (Phase 9-bis-2 final-cleanup): intentional discrepancy by design.** Provider value (₹10L) is the ADR 0026 framework-readiness contract; legacy `sandbox/fund_manager.py` (₹1Cr) is the operator-configurable runtime default. Both pinned by their own tests. The discrepancy is now explicitly documented in the provider source. |
| ~~T-17 (Flow constants + 14 flow nodes)~~ | ~~v9-bis-2 Phase 4-bis-2~~ | **CLOSED 2026-05-02 via relocation (Phase 4-bis-2 + earlier v9-bis frontend pass).** All flow constants + 19 flow nodes were relocated to `frontend/src/india_legacy/components/flow/` and `frontend/src/india_legacy/lib/flow/`. The relocation IS the answer: these nodes encode India-specific operations (OptionChain on NSE, OptionsMultiOrder with NFO grammar, etc.) and migrating them to be region-capable would require a different React layer entirely. Acknowledged as INTENTIONALLY-INDIA-ONLY in the india_legacy/ subtree. |
| ~~T-18 (`strategyMath.ts` type-system migration)~~ | ~~v9-bis-2 Phase 4-bis-2~~ | **CLOSED 2026-05-02 via relocation.** `strategyMath.ts` and `strategyTemplates.ts` were relocated to `frontend/src/india_legacy/lib/`. The math is intrinsically tied to India options grammar (CE/PE notation, DDMMMYY expiry parsing, 15:30 IST expiry-close close). Migrating to a generic `OptionRight = 'CALL' \| 'PUT'` would have to rebuild the entire strategy-builder pipeline. Relocation is the appropriate answer. |
| ~~T-23 physical relocation (37 files into `market_regions/india/legacy_v1/`)~~ | ~~v9-bis-2 Phase 9~~ | **CLOSED 2026-05-02 (Phase 9-bis-physical).** Moved to Implemented bucket above. 44 files relocated (slightly more than the 37 estimate), Option B shim layer used per the playbook. |
| ~~T-24 full — relocate per-section endpoint pages under `docs/api/v1/<section>/`~~ | ~~v9-bis-2 Phase 8-bis-2~~ | **CLOSED 2026-05-02 (Phase 8-bis-2-2).** Moved to Implemented bucket above. Per-file redirect stubs preserve external deep-link compatibility. |
...
| ~~T-35 (LEGACY_INDIA bucket < 50)~~ | ~~v9-bis-2 Phase 9 (partial)~~ | **CLOSED 2026-05-02 (Phase 9-bis-2 final-cleanup): LEGACY_INDIA reduced from 211 → 49.** Achieved via mass relocation under `market_regions/india/legacy_v1/` (sandbox engine, options/analytics/flow/historify/sandbox/analyzer/telegram/chart services, India-only blueprints, India-only DB modules, India-only utils) + promotion of region-neutral utilities/services/DB modules to PROMOTED_CORE. T-35 acceptance criterion met. |
| ~~Phase 4-bis-2 — ConfigPanel, PlaceOrderDialog, MarketTimings, CustomStraddle, scheduler-tied pages~~ | ~~v9-bis-2 Phase 4-bis-2~~ | **MOVED to "Phase 4-bis-2 — frontend UX" entry above (deferred to a frontend-UX-engagement that needs browser verification).** |
| ~~Phase 2-bis-2 — sandbox catch_up/holdings/execution_engine; 6 remaining options services; chartink legacy parser; flow_scheduler/python_strategy/historify_scheduler venue-aware~~ | ~~v9-bis-2 Phase 2-bis-2~~ | **CLOSED 2026-05-02 via relocation (Phase 9-bis-2 final-cleanup).** All these modules were relocated under `market_regions/india/legacy_v1/` as INTENTIONALLY-INDIA-ONLY. The dispatcher-wiring goal of Phase 2-bis-2 was to make consumers route through dispatcher contracts, which is now true: the 4 restx_api/option_*.py + analyzer_service + sandbox_service all dispatch through `get_options_provider("india").service_modules()` / `get_sandbox_provider("india").service_modules()` (Phase 2-bis-3-3). The relocated India-only services keep their existing internal logic (still IST-bound, still NSE/NFO-shaped) — their classification under `legacy_v1/` ratifies that as the correct architectural answer. |
| Phase 4-bis-2 — master-contract refresh-policy execution + rule_enforcement.check_order entitlement integration | DEFERRED — requires real broker plugins | These two are broker-side runtime concerns: master-contract refresh-policy is per-broker capability metadata that's only exercised when a real broker plugin's daily refresh runs; rule_enforcement.check_order entitlement integration is enabled per real broker. Both are blocked on real Schwab/Webull/Alpaca/IBKR plugins (which are themselves blocked on official API access). The **framework** is ready: `BrokerCapabilities.master_contract_refresh_policy` is a required field for promoted plugins (ADR 0017); `services.rule_enforcement.check_order` exists and is dispatcher-driven. The runtime activation lands when a real plugin lands. |
| Phase 4-bis-2 — ConfigPanel, PlaceOrderDialog, MarketTimings, CustomStraddle, scheduler-tied pages browser-verified per-component migration | v9-bis-2 Phase 4-bis-2 (frontend UX) | These are frontend per-component UX migrations beyond the literal-scanner perimeter (already drained to 2 entries). Each requires actual browser verification of the rendered UI, not achievable in a pure code session. Acknowledged as a frontend-UX-engagement deferred item. |
