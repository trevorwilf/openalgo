"""Hybrid shim for ``services.telegram_bot_service_fixed``.

Both forms used here are necessary:

* ``_sys.modules[__name__] = _orig`` preserves module identity so
  ``mock.patch.object(services.telegram_bot_service_fixed, "X", ...)`` and other tests
...
* The post-swap attribute copy ensures any caller that looks up
  ...
  ``from services.telegram_bot_service_fixed import X`` inside a Flask route that
  resolved to the shim before the swap settled) still finds them.

Source of truth:
``market_regions.india.legacy_v1.services.telegram_bot_service_fixed``.
"""
from __future__ import annotations

import sys as _sys

from market_regions.india.legacy_v1.services import telegram_bot_service_fixed as _orig
...
# future imports of ``services.telegram_bot_service_fixed`` resolve to ``_orig`` and so
