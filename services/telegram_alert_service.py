"""Hybrid shim for ``services.telegram_alert_service``.

Both forms used here are necessary:

* ``_sys.modules[__name__] = _orig`` preserves module identity so
  ``mock.patch.object(services.telegram_alert_service, "X", ...)`` and other tests
...
* The post-swap attribute copy ensures any caller that looks up
  ...
  ``from services.telegram_alert_service import X`` inside a Flask route that
  resolved to the shim before the swap settled) still finds them.

Source of truth:
``market_regions.india.legacy_v1.services.telegram_alert_service``.
"""
from __future__ import annotations

import sys as _sys

from market_regions.india.legacy_v1.services import telegram_alert_service as _orig
...
# future imports of ``services.telegram_alert_service`` resolve to ``_orig`` and so
