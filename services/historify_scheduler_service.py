"""Hybrid shim for ``services.historify_scheduler_service``.

Both forms used here are necessary:

* ``_sys.modules[__name__] = _orig`` preserves module identity so
  ``mock.patch.object(services.historify_scheduler_service, "X", ...)`` and other tests
...
* The post-swap attribute copy ensures any caller that looks up
  ...
  ``from services.historify_scheduler_service import X`` inside a Flask route that
  resolved to the shim before the swap settled) still finds them.

Source of truth:
``market_regions.india.legacy_v1.services.historify_scheduler_service``.
"""
from __future__ import annotations

import sys as _sys

from market_regions.india.legacy_v1.services import historify_scheduler_service as _orig
...
# future imports of ``services.historify_scheduler_service`` resolve to ``_orig`` and so
