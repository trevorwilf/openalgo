"""v6 Phase 7 — Kotak Neo v2 broker translator."""
...
class KotakOrderTranslator:
    ...
    def to_native(self, order: Any, instrument: Any, account_ctx: AccountContext) -> dict[str, Any]:
        ...
        return {
            "tk": str(_ifield(instrument, "broker_token", "token") or ""),
            "es": str(_ifield(instrument, "venue_code", "exchange") or "NSE").lower() + "_cm",
            "ts": str(_ifield(instrument, "broker_symbol", "canonical_symbol") or "X"),
            "qt": str(int(order.quantity)),
            "pr": str(order.price) if order.price is not None else "0",
            "tp": str(order.trigger_price) if order.trigger_price is not None else "0",
            "tt": order.side.value.upper()[:1],  # B / S
            "pc": _native_product(order),
            "rt": _TIF_MAP[order.time_in_force],
            "pt": _ORDER_TYPE_MAP[order.order_type],
            "mp": "0",
            "dq": "0",
            "am": "NO",
        }
    ...
    def from_native_order_response(self, payload: dict[str, Any], instrument: Any) -> dict[str, Any]:
        order_id = payload.get("nOrdNo") or payload.get("orderId") or payload.get("order_id")
        if order_id is None:
            raise ValueError(f"Kotak order response missing nOrdNo: {payload!r}")
        return {
...
def install_kotak_translator() -> KotakOrderTranslator:
    from services.broker_translator_registry import register_broker_translator

    t = KotakOrderTranslator()
    register_broker_translator(t)
    return t


__all__ = ["BROKER_CODE", "KotakOrderTranslator", "install_kotak_translator"]
