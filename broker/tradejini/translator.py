"""v6 Phase 7 — Tradejini v2 broker translator."""
...
_PRODUCT_MAP: dict[str, str] = {"CNC": "delivery", "NRML": "normal", "MIS": "intraday"}
...
class TradejiniOrderTranslator:
    ...
    def to_native(self, order: Any, instrument: Any, account_ctx: AccountContext) -> dict[str, Any]:
        ...
        return {
            "exchange": str(_ifield(instrument, "venue_code", "exchange") or "NSE").lower(),
            "tradingSymbol": str(_ifield(instrument, "broker_symbol", "canonical_symbol") or "X"),
            "instrumentToken": str(_ifield(instrument, "broker_token", "token") or ""),
            ...
            "validity": _TIF_MAP[order.time_in_force].lower(),
            "amo": False,
    ...
    def from_native_order_response(self, payload: dict[str, Any], instrument: Any) -> dict[str, Any]:
        ...
        if order_id is None:
            raise ValueError(f"Tradejini order response missing orderId: {payload!r}")
        return {
...
def install_tradejini_translator() -> TradejiniOrderTranslator:
    from services.broker_translator_registry import register_broker_translator

    t = TradejiniOrderTranslator()
    register_broker_translator(t)
    return t


__all__ = ["BROKER_CODE", "TradejiniOrderTranslator", "install_tradejini_translator"]
