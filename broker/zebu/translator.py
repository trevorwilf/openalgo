"""v6 Phase 7 — Zebu (Noren) v2 broker translator."""
...
class ZebuOrderTranslator:
    ...
    def from_native_order_response(self, payload: dict[str, Any], instrument: Any) -> dict[str, Any]:
        ...
        if order_id is None:
            raise ValueError(f"Zebu order response missing norenordno: {payload!r}")
        return {
...
def install_zebu_translator() -> ZebuOrderTranslator:
    from services.broker_translator_registry import register_broker_translator

    t = ZebuOrderTranslator()
    register_broker_translator(t)
    return t


__all__ = ["BROKER_CODE", "ZebuOrderTranslator", "install_zebu_translator"]
