"""v6 Phase 7 — Pocketful v2 broker translator."""
...
class PocketfulOrderTranslator:
    ...
    def to_native(self, order: Any, instrument: Any, account_ctx: AccountContext) -> dict[str, Any]:
        ...
        return {
            "client_id": str(account_ctx.account_id),
    ...
    def from_native_order_response(self, payload: dict[str, Any], instrument: Any) -> dict[str, Any]:
        ...
        order_id = data.get("oms_order_id") or data.get("order_id") or data.get("orderId")
        if order_id is None:
            raise ValueError(f"Pocketful order response missing oms_order_id: {payload!r}")
        return {
...
def install_pocketful_translator() -> PocketfulOrderTranslator:
    from services.broker_translator_registry import register_broker_translator

    t = PocketfulOrderTranslator()
    register_broker_translator(t)
    return t


__all__ = ["BROKER_CODE", "PocketfulOrderTranslator", "install_pocketful_translator"]
