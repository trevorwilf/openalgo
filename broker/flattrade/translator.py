"""v6 Phase 6 — Flattrade v2 broker translator.

Flattrade uses Noren / pi-connect API. Native shape similar to
Firstock/Shoonya: prctyp (MKT/LMT/SL-LMT/SL-MKT) + prd (C/M/I) +
trantype (B/S).

The legacy v1 path also applies Market Price Protection (MPP) for
MARKET orders. The v2 translator does NOT replicate the MPP — that
is operational logic at the dispatcher / order_api level, not
translation. v2 callers that want MPP-protected market orders should
submit a LIMIT order with the protected price. (Bit-identical with
v1 *for any limit/stop submission*; market-order protection is an
operational concern handled outside the translator.)
"""
...
class FlattradeOrderTranslator:
    ...
    def from_native_order_response(self, payload: dict[str, Any], instrument: Any) -> dict[str, Any]:
        ...
        if order_id is None:
            raise ValueError(f"Flattrade order response missing norenordno: {payload!r}")
        return {
...
def install_flattrade_translator() -> FlattradeOrderTranslator:
    from services.broker_translator_registry import register_broker_translator

    t = FlattradeOrderTranslator()
    register_broker_translator(t)
    return t


__all__ = ["BROKER_CODE", "FlattradeOrderTranslator", "install_flattrade_translator"]
