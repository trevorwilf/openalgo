"""v6 Phase 6 — Firstock v2 broker translator."""
...
class FirstockOrderTranslator:
    ...
    def to_native(self, order: Any, instrument: Any, account_ctx: AccountContext) -> dict[str, Any]:
        ...
        return {
            "userId": str(account_ctx.account_id),
            ...
            "priceType": _PRICE_TYPE_MAP[order.order_type],
            "retention": _TIF_MAP[order.time_in_force],
            "remarks": order.strategy_tag or "Place Order",
        }
    ...
    def from_native_order_response(self, payload: dict[str, Any], instrument: Any) -> dict[str, Any]:
        ...
        if order_id is None:
            raise ValueError(f"Firstock order response missing orderNumber: {payload!r}")
        return {
...
def install_firstock_translator() -> FirstockOrderTranslator:
    from services.broker_translator_registry import register_broker_translator

    t = FirstockOrderTranslator()
    register_broker_translator(t)
    return t


__all__ = ["BROKER_CODE", "FirstockOrderTranslator", "install_firstock_translator"]
